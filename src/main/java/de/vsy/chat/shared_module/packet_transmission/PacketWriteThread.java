...
import de.vsy.chat.shared_module.packet_management.InputBuffer;
...
class PacketWriteThread extends ThreadContextRunnable {
     ...
     */
    public
    PacketWriteThread (final UnconfirmedPacketTransmissionCache packetCache,
                       final OutputStream out, final InputBuffer queueAccess)
    throws IOException {
     ...
     */
    private
    void setupOutputStream (final OutputStream out)
    throws IOException {
     ...
     */
    private
    void writeContent (final Packet output) {

        try {
            this.outStream.writeObject(output);
            this.outStream.flush();
            LOGGER.debug("Geschrieben: {}", output);
