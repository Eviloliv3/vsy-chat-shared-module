package de.vsy.shared_module.shared_module.packet_transmission.cache;
...
/**
 * Wrapper for Packet, that provides a creation time, a cycle time and a cycle
 * counter indicating the time that has passed since the Wrapper creation. Frederic
 * Heath
 */
public
class CachedPacket {
     ...
     */
    public
    CachedPacket (final Packet toCache, final int cachingCycleMillis) {
    ...
    public
    void increasePassedCycles () {
        final Instant currentTime = Instant.now();
        this.cyclesPassed = (int) (
                Duration.between(this.cachingTime, currentTime).toMillis() /
                this.cycleMillis);
    }
}
